...
# Change for Channel-wise Attention
...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Channel-wise Attention            &   original
        y = torch.sigmoid(self.fc2(y))      #   y = torch.tanh(self.fc2(y)) 
...
# ----Change Active function of MSEF to LeakyReLU---------------------------------------------------
...
#     def _init_weights(self):
#         # replace as LeakyReLU
#         init.kaiming_uniform_(self.fc1.weight, a=0.01, mode='fan_in', nonlinearity='leaky_relu')
#         init.constant_(self.fc1.bias, 0)
...
# ----Add Swish activation function to MHSA--------------------------------------------------------------------------------
...
#         self.key_dense = nn.Linear(embed_size, embed_size)
#         self.value_dense = nn.Linear(embed_size, embed_size)
#         self.swish = nn.SiLU()  # Swish
#         self.combine_heads = nn.Linear(embed_size, embed_size)
#         self._init_weights()

#     def split_heads(self, x, batch_size): 
#         # Split the last dimension into (num_heads, head_dim)
#         # and reshape the result to (batch_size, num_heads, seq_length, head_dim) for multi-head computation
#         x = x.reshape(batch_size, -1, self.num_heads, self.head_dim)
#         # Swap dimensions to fit the requirements of the attention operation
#         return x.permute(0, 2, 1, 3) 


#     def _init_weights(self):
#         for layer in [self.query_dense, self.key_dense, self.value_dense, self.combine_heads]:
#             init.xavier_uniform_(layer.weight)  # Xavier initialization for linear layers with Swish
...
#         query = self.split_heads(self.swish(self.query_dense(x)), batch_size)
#         key = self.split_heads(self.swish(self.key_dense(x)), batch_size)
#         value = self.split_heads(self.swish(self.value_dense(x)), batch_size)
        
#         attention_weights = F.softmax(torch.matmul(query, key.transpose(-2, -1)) / (self.head_dim ** 0.5), dim=-1)
#         attention = torch.matmul(attention_weights, value)
#         attention = attention.permute(0, 2, 1, 3).contiguous().reshape(batch_size, -1, self.embed_size)
        
#         output = self.swish(self.combine_heads(attention))
...
class Denoiser(nn.Module):
    def __init__(self, num_filters, kernel_size=3, activation='relu'):
        ...
        # Channel-wise Attention                    &   original
        self.bottleneck = SEBlock(num_filters)      #   self.bottleneck = MultiHeadSelfAttention(embed_size=num_filters, num_heads=4)
...
# ----Change activation function of CWD to PReLU------------------------------------------------------------------

# class Denoiser(nn.Module):
#     def __init__(self, num_filters, kernel_size=3):
#         super(Denoiser, self).__init__()
#         self.conv1 = nn.Conv2d(1, num_filters, kernel_size=kernel_size, padding=1)
#         self.prelu1 = nn.PReLU()  # add PReLU activation function
#         self.conv2 = nn.Conv2d(num_filters, num_filters, kernel_size=kernel_size, stride=2, padding=1)
...
#         x2 = self.prelu2(self.conv2(x1))
#         x3 = self.prelu3(self.conv3(x2))
#         x4 = self.prelu4(self.conv4(x3))
#         x = self.bottleneck(x4)
#         x = self.up4(x)
#         x = self.up3(x + x3)
#         x = self.up2(x + x2)
...
#     def _init_weights(self):
#         for layer in [self.conv1, self.conv2, self.conv3, self.conv4, self.res_layer]:
#             # Considering the use of PReLU activation, Leaky ReLU properties are used for weight initialization
...
class LYT(nn.Module):
    def __init__(self, filters=32):
        ...
        # Channel-wise Attentio             &   original
        self.lum_attn = SEBlock(filters)    #   self.lum_mhsa = MultiHeadSelfAttention(embed_size=filters, num_heads=4)
